...
class Controller:
    ...
    def __init__(
        self, src_host, src_port, dest_port, broadcast_host, number_of_servers
    ):
        ...
        self.sockets = custom_socket.Sockets(
            self.src_host,
            self.src_port,
        ...
        threading.Thread(
            target=self.start_animation, args=("Checking cameras",)
        ).start()
        ...
        threading.Thread(
            target=self.stop_animation, args=(self.main_view.frame_main, pool1)
        ).start()
    ...
    def start_animation(self, label_text, animation=["", ".", "..", "..."]):
        ...
        self.loading_label = tk.Label(
            self.loading_frame,
            text="{}{}".format(self.label_text, animation[0]),
            font=self.main_view.font_title,
        )
        self.loading_label.place(anchor="center", relx=0.5, rely=0.5)
        ...
        def iterate(self):
            ...
            self.loading_label.config(
                text="{}{}".format(self.label_text, next(self.iter_animation))
            )
    ...
    def check_connections(self, timeout):
        ...
        while len(connection_list) < self.number_of_servers:
            try:
                ...
                logger.info(
                    "Timed out waiting on initial connections. Connected to:",
                    connection_list,
                )
                break
        pool2.close()
    ...
    def view_connections(self):
        ...
        messagebox.showinfo(
            "Live Connections", "Connected to camera(s): {}".format(connections)
        )
    ...
    def request_take_photos(self):
        ...
        while True:
            try:
                ...
                logger.info(
                    "All photo taken responses received. Socket timed out and closed."
                )
                break
    ...
    def request_photo_files_then_load(self):
        ...
        while True:
            try:
                ...
                self.server_dict[pi_id] = {
                    "photo_path": write_location,
                    "ip_address": ip,
                }
                pool3.apply_async(
                    self.sockets.recv_tcp_data, (conn, 8192, True, write_location)
                )
            except Exception as e:
                logger.info(
                    "All incoming connections trying to send photos have been accepted"
                )
                break
        pool3.close()
        pool3.join()
        ...
        for pi_id in self.server_dict:
            ...
            photo = Image.open(self.server_dict[pi_id]["photo_path"]).resize(
                (275, 206), Image.ANTIALIAS
            )
        ...
        if len(photo_list) > 0:
            ...
            messagebox.showinfo(
                "Photo Error", "No photos could be retrieved, please try again."
            )
    ...
    def confirm_upload(self):
        answer = messagebox.askyesno(
            title="Confirm Upload", message="Confirm photo upload?"
        )
        return answer
    ...
    def upload(self):
        ...
        def inner_upload(self, pi_id):
            try:
                ...
                dbx.files_upload(
                    photo.read(), "/Pet_Pics/{}/img{}.jpg".format(folder_name, pi_id)
                )
                photo.close()
            except Exception:
                traceback.print_exc()
    ...
    def create_counter_labels(self, number_of_photos, starting_computer_id):
        self.label_counter = tk.Label(
            self.review_view.frame_review,
            text="1/{}".format(number_of_photos),
            font=self.review_view.font_buttonH2,
        )
        self.label_counter.place(anchor="center", relx=0.5, rely=0.13)
        self.label_pi_num = tk.Label(
            self.review_view.frame_review,
            text="#{}".format(starting_computer_id),
            font=self.review_view.font_unimportant,
        )
        self.label_pi_num.place(anchor="center", relx=0.75, rely=0.13)
...
def main(src_host, src_port, dest_port, broadcast_host, number_of_servers):
    main_controller = Controller(
        src_host, src_port, dest_port, broadcast_host, number_of_servers
    )
